...
class AttendanceRecapController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($view === 'majors') {
            $majors = Major::withCount(['studentClasses as student_classes_count']) // Count classes per major
                ->when($search, function($query) use ($search) {
                    $query->where('name', 'like', "%{$search}%");
                })
                ->get();
            
            // To get active students count for each major, we need a separate aggregation or a more complex relation.
            // For now, student_classes_count is the count of classes.
            $data = $majors;
            ...
            $classes = StudentClass::where('major_id', $majorId)
                ->withCount(['students as active_students_count' => function($query) { // Added withCount for active students
        ...
        return view('admin.attendance-recap.index', compact(
            'data', 'view', 'majorId', 'classId', 'search', 'breadcrumb', 'date', 'noRecapMessage'
        ));
    }
    ...
    public function update(Request $request)
    {
        ...
        $request->validate([
            // Mengubah 'student_classes' menjadi 'classes' sesuai nama tabel di database Anda
