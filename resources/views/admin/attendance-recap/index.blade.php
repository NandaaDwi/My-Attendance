{{-- resources/views/admin/attendance-recap/index.blade.php --}}
        ...
        <div class="flex flex-col lg:flex-row gap-4 lg:items-center lg:justify-between">
            ...
            <div class="flex-shrink-0" x-show="currentView !== 'majors'">
                ...
                <template x-if="currentView === 'students'">
                    <a :href="`{{ route('admin.attendance-recap.index') }}?view=classes&major_id=${currentMajorId}`"
                        class="inline-flex items-center justify-center bg-gray-600 hover:bg-gray-700 text-white px-6 py-3 rounded-xl transition-all duration-200 font-medium">
                        <i class="fas fa-arrow-left mr-2"></i>
                        Kembali ke Kelas
                    </a>
    ...
    <div id="content-container">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                <template x-for="major in data" :key="major.id">
                    <div
                        ...
                        <a :href="`{{ route('admin.attendance-recap.index') }}?view=classes&major_id=${major.id}`"
                            class="block">
        ...
        <template x-if="currentView === 'classes'">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                <template x-for="cls in data" :key="cls.id">
                    <div
                        class="group bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 hover:scale-105">
                        <a :href="`{{ route('admin.attendance-recap.index') }}?view=students&major_id=${currentMajorId}&class_id=${cls.id}`"
                            class="block">
                            <div class="p-6 text-center">
                                <div
        ...
        <template x-if="currentView === 'students'">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                {{-- Menggunakan x-show untuk mengontrol visibilitas pesan "Tidak ada data" --}}
                <div x-show="data.length === 0" class="col-span-full text-center py-16">
                    <div class="w-16 h-16 mx-auto bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center mb-4">
                        <i class="fas fa-box-open text-gray-400 text-2xl"></i>
                    </div>
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">Tidak ada data absensi ditemukan</h3>
                    <p class="text-gray-600 dark:text-gray-400" x-text="noRecapMessage"></p>
                </div>

                {{-- Menggunakan x-show untuk mengontrol visibilitas tabel --}}
                <div x-show="data.length > 0" class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            <template x-for="(record, index) in data" :key="record.student_id">
                                {{-- Menghapus log yang menyebabkan error "record is not defined" --}}
                                <tr>
...
<script>
    ...
    function attendanceRecapApp(initialState) {
        ...
        return {
            ...
            noRecapMessage: initialState.initialNoRecapMessage,
            ...
            init() {
                ...
                if (this.currentView === 'students' && initialState.initialClassId) {
                    // Jika data awal ada, dapatkan nama kelas dari catatan pertama
                    if (this.data.length > 0) {
                        this.classDisplayName = this.data[0].class_name || 'N/A';
                    } else {
                        // Jika tidak ada data awal (misalnya, tidak ada siswa aktif), ambil nama kelas
            ...
            async fetchClassNameFromController(classId) {
                ...
                try {
                    const res = await fetch(`{{ route('admin.attendance-recap.index') }}?${params.toString()}`, {
                        headers: {
            ...
            async performSearch() {
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        this.noRecapMessage = data.noRecapMessage;
            ...
            async saveChanges() {
                ...
                try {
                    const response = await fetch(`{{ route('admin.attendance-recap.update') }}`, {
                        method: 'POST',
                        headers: {
